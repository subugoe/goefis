	    ...
	    init: function() {
	        ...
	        this.on("success", function(file,response){
	            ...
	            if(resp.success){
	              ...
	              var fileName = Dropzone.createElement("<div class=\"row\"><div class=\"col-md-12 padded text-muted\" id=\"filename_" + resp.tempid + "\"><span class=\"glyphicon glyphicon-file text-muted\"></span> " + file.name + "</div></div>");
	              file.previewElement.appendChild(fileName);
		...
		init: function(){
	        ...
	        this.on("uploadprogress", function(file,progress,bytesSent){
		   	...
		   	this.on("success", function(file,response){
	            ...
	            var file_name = Dropzone.createElement("<h4 class='expert'>" + file.name + "</h4>");
